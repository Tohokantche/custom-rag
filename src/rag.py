...
from src.utils import LoadReRanker
...
class RagLogic:
    ...
    def process_question_multi_pdf(
        ...
        selected_model: str,
        chat_context,
        re_ranker : LoadReRanker.ReRanker = None
    ) -> Tuple[str, List[Dict]]:
        ...
        QUERY_PROMPT = PromptTemplate(
            ...
            different versions of the given user question to retrieve only relevant documents from
            a vector database. By generating multiple semantically faithful perspectives on the user question, your
            goal is to help the user overcome some of the limitations of the distance-based
            similarity search. Provide these alternative questions separated by newlines, and stick as much as
            you can to the semantic of the original question without hallucinating.
        ...
        # Add re-rankers
        if re_ranker:
            logger.info(f"Re-ranking documents")
            re_rank_documents = re_ranker.get_ranked_document(temp_querry_docs_pairs, temp_sources)
            for score, doc, source in re_rank_documents[:10]:  # Top 10 chunks
